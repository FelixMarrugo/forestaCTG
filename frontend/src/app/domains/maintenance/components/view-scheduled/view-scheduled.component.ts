...
export class ViewScheduledComponent implements OnInit {
  ...
  scheduledMaintenanceEdit!: FormGroup;
  ...
  async editScheduledMaintenance() {
    ...
    if (this.scheduledMaintenanceEdit.valid) {
      ...
      this.maintenanceService.updateScheduledMaintenance(id, description).subscribe({
        next: async (response) => {
          const alert = await this.alertController.create({
            header: '¡Todo bien, compa!',
            message: 'Mantenimiento programado actualizado exitosamente.',
            buttons: ['OK'],
